...
def train_model(hidden_layers):
    ...
    model.add(tensorflow.keras.layers.Dense(100, activation='relu', input_shape=(784,)))  # 输入层到第一个隐含层
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('task3_training_test_loss.png')  # 保存图像
plt.show()
...
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('task3_training_test_accuracy.png')  # 保存图像
plt.show()
...
for i, layers_count in enumerate(hidden_layer_counts):
    ...
    plot_confusion_matrix(y_test, y_pred, f'Confusion Matrix for {layers_count} Hidden Layers', f'task3_confusion_matrix_{layers_count}_layers.png')
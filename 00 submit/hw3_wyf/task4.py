...
def train_model(hidden_units):
    ...
    model.add(tensorflow.keras.layers.Dense(hidden_units, activation='sigmoid', input_shape=(784,)))  # 输入层到第一个隐含层
    ...
    # 计算运行时间
    end_time = time.time()
    print(f"Training with {hidden_units} hidden units completed in {end_time - start_time:.2f} seconds.")

    # 评估模型
    test_results = model.evaluate(X_test, y_test, verbose=0)
    print(f'Test accuracy with {hidden_units} hidden units: {test_results[1]:.4f}')


    return train_loss, train_acc, test_loss, test_acc, model

# 隐含层单元数列表
hidden_unit_counts = [100, 300, 500]
...
for hidden_units in hidden_unit_counts:
    train_loss, train_acc, test_loss, test_acc, model = train_model(hidden_units)
...
for i, hidden_units in enumerate(hidden_unit_counts):
    plt.plot(train_losses[i], label=f'{hidden_units} Hidden Units (Train Loss)')
    plt.plot(test_losses[i], label=f'{hidden_units} Hidden Units (Test Loss)')

plt.title('Training and Test Loss')
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.legend()
plt.savefig('task4_training_test_loss.png')  # 保存图像
plt.show()
...
for i, hidden_units in enumerate(hidden_unit_counts):
    plt.plot(train_accuracies[i], label=f'{hidden_units} Hidden Units (Train Accuracy)')
    plt.plot(test_accuracies[i], label=f'{hidden_units} Hidden Units (Test Accuracy)')

plt.title('Training and Test Accuracy')
plt.xlabel('Epochs')
plt.ylabel('Accuracy')
plt.legend()
plt.savefig('task4_training_test_accuracy.png')  # 保存图像
plt.show()
...
for i, hidden_units in enumerate(hidden_unit_counts):
    ...
    plot_confusion_matrix(y_test, y_pred, f'Confusion Matrix for {hidden_units} Hidden Units', f'task4_confusion_matrix_{hidden_units}_Units.png')